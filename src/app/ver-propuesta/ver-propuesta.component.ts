...
export class VerPropuestaComponent implements OnInit {
  ...
  constructor(
    ...
    this.propuestaForm = this.fb.group({
      montApprove: new FormControl(''),
      propuesta_detalle: this.fb.group({
        fechaPropuesta: new FormControl('', Validators.required),
        fechaUltima: new FormControl('', Validators.required),
        fechaProxima: new FormControl('', Validators.required),
        idPropuesta: new FormControl('', Validators.required),
        numPropuesta: new FormControl('', Validators.required),
        grupoEconomico: new FormControl('', Validators.required),
        numGrupoEconomico: new FormControl('', Validators.required),
        calificacionScoreRate: new FormControl('', Validators.required),
        idActividad: new FormControl('', Validators.required),
        sector: new FormControl('', Validators.required),
        parteRelacionada: new FormControl('', Validators.required),
        clasificacionRiesgo: new FormControl('', Validators.required),
        provisionSIB: new FormControl('', Validators.required),
        provisionNIIF: new FormControl('', Validators.required),
        unidadResponsable: new FormControl('', Validators.required),
        centroCosto: "",
        ejecutivoCuenta: new FormControl('', Validators.required),
        aprobacionControl: "",
      }),
    ...
    this.revisorForm = this.fb.group({
      idUsuario: "",
      idRevisor: new FormControl('', [Validators.required]),
      fechaRevision: "",
      estadoRevision: ""
    });
  ...
  public ngOnInit(): void {
    ...
    forkJoin(
      ...
      this.cargo.getDataApprove(),
      ).subscribe(([PropuestaID, U, R, P, F, E, ME, A, NA, TSP, TSF, PROPO, GARA, FIAN, REFI, CAR, CARSINUNIDAD, LISTABROVE]) => {
      ...
      this.listApprove = LISTABROVE;
      
      console.log(CARSINUNIDAD);
      console.log(LISTABROVE);
    }, error => {
      console.error(error);
    },
      () => {
  ...
  public iniciar(): void {
    ...
    try {
      ...
      if (iE == 14 || iE == 15) {
        ...
        console.log("Estado: " + nE);
        this.revisorForm.get("estadoRevision").setValue(nE);
      }
      let d = this.propDevuelta.FechaRevision;
  ...
  //   const formProducts: FormArray = this.formProducts();
  //   formProducts.push(this.initFormProduct());
  // }


  // public formFacilidades(): FormArray {

  //   return (this.inventoryForm.controls.products as FormArray);
  // }
  ...
  public get propuestaDetalleForm(): FormGroup {

    return this.propuestaForm.controls['propuesta_detalle'] as FormGroup;
  }

  public guardarPropuesta(): void {
    

    let dat: string = 'Guardar datos de la propuesta';
    ...
    memoRef.afterClosed().subscribe(result => {
      if (result == true) {
        ...
        if (this.ids != null) {
          this.putPropuesta();
          this.defaultToastService.success('Guardado Exitoso', 'Se ha guardo correctamente');
  ...
  enviarPropuesta() {
    let idRevisor = this.revisorForm.get("idRevisor").value;
    let idUsuario: string = JSON.parse(localStorage.getItem('id'));

    let dat: string = 'Enviar al revisor';
    let tit: string = this.headerProp;

    if (this.revisorForm.get("idRevisor") != undefined &&
      this.revisorForm.get("idRevisor") != null &&
      this.revisorForm.get("idRevisor").value != '00000000-0000-0000-0000-000000000000' &&
      this.revisorForm.get("idRevisor").value != '') {
      ...
      memoRef.afterClosed().subscribe(result => {
        if (result == true) {
          ...
          this.estadopropuesta.putDataEstadoPropuesta9(cam).subscribe(
            res => (console.log(res)),
            err => { },
            () => {
